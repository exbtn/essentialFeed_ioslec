...
final class RemoteFeedLoaderTests: XCTestCase {
    ...
    func test_load_deliversNoItemsOn200HTTPResponseWithEmptyListJSON() {
        ...
        expect(sut, toCompleteWith: .success([]), when: {
            let emptyListJSON = makeItmesJSON([])
            client.complete(withStatusCode: 200, data: emptyListJSON)
        })
    }
    
    func test_load_deliversItemsOn200HTTPResponseWithValidItemsJSON() {
        let (sut, client) = makeSUT()
        ...
        expect(sut, toCompleteWith: .success([item1.model, item2.model]), when: {
            let validItemsJSON = makeItmesJSON([item1.json, item2.json])
            client.complete(withStatusCode: 200, data: validItemsJSON)
        })
    }
    ...
    func test_load_doesNotDeliverResultAfterSUTInstanceHasBeenDeallocated() {
        ...
        sut = nil
        client.complete(withStatusCode: 200, data: makeItmesJSON([]))
    ...
    private func makeItmesJSON(_ items: [[String: Any?]]) -> Data {
