...
final class CodableFeedStoreTests: XCTestCase, FailabeFeedStore {
    ...
    func test_retrieve_hasNoSideAffectsOnNonEmptyCache() {
        ...
        assertThatRetrieveHasNoSideAffectsOnNonEmptyCache(on: sut)
    }
    ...
    func test_delete_deliversErrorOnDeletionFailure() {
        ...
        XCTAssertNotNil(deletionError, "Expected to cache deletion fail")
    }
    
    func test_delete_hasNoSideEffectsOnDeletionFailure() {
        let noDeletePermissionURL = cachesDirectory()
        let sut = makeSUT(storeURL: noDeletePermissionURL)
