...
final class EssentialFeedAPIEndToEndTests: XCTestCase {
    ...
    func test_endToEndTestServerGETFeedResult_mathcesFixedTestAccountData() {
        ...
        case let .failure(error)?:
            XCTFail("Expected to receive success, got a \(error)")
            
        default:
            XCTFail("Expected to receive success, got no result")
        }
    }
    ...
    private func locaion(at index: Int) -> String? {
        return [
            "Location 1",
            "Location 2",
            nil,
